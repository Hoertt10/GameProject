...
public class defalue : MonoBehaviour {
    ...
    public void ShowRandom()
    {
        foreach (int Arr in RandomArr)
        {
            ...
            objlist[Arr-1].GetComponent<SpriteRenderer>().sprite = Resources.Load("black", typeof(Sprite)) as Sprite;
        }
        DelayRecover();
    }
    ...
    IEnumerator delay()
    {
        ...
        {
            //Debug.Log(Arr);
            objlist[Arr-1].GetComponent<SpriteRenderer>().sprite = Resources.Load("white", typeof(Sprite)) as Sprite;
        }
    ...
    public static void AnsList(string name)
    {
        if (name != null)
        {
            ...
            if (AnsCover)
            {
                GameObject.Find(AnsArr[i]).GetComponent<SpriteRenderer>().sprite = Resources.Load("white", typeof(Sprite)) as Sprite;
            }
            AnsArr[i] = name;
            GameObject.Find(AnsArr[i]).GetComponent<SpriteRenderer>().sprite = Resources.Load("chack", typeof(Sprite)) as Sprite;
            i++;
